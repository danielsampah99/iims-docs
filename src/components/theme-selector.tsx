...
export function ThemeSelector(props: React.ComponentPropsWithoutRef<typeof Listbox<'div'>>) {
	...
	return (
		...
		>
			...
			>
				<LightIcon
					className={cn('h-4 w-4 dark:hidden', theme === 'system' ? 'fill-slate-400' : 'fill-sky-400')}
				/>
				<DarkIcon
					className={cn('hidden h-4 w-4 dark:block', theme === 'system' ? 'fill-slate-400' : 'fill-sky-400')}
				/>
			</ListboxButton>
			<ListboxOptions className="absolute max-sm:left-1/2 left-1/2 top-full mt-3 max-sm:w-10 w-36 -translate-x-1/2 space-y-1 max-sm:p-1 rounded-xl bg-white p-3 text-sm font-medium shadow-md shadow-black/5 ring-1 ring-black/5 dark:bg-slate-800 dark:ring-white/5">
				{themes.map((theme) => (
					<ListboxOption
						key={theme.value}
						value={theme.value}
						className={({ focus, selected }) =>
							cn('flex cursor-pointer select-none items-center rounded-[0.625rem] p-1', {
					...
					>
						{({ selected }) => (
							<>
								...
								<div className="ml-3 max-sm:ml-0 max-sm:hidden">{theme.name}</div>
							</>
						)}
					</ListboxOption>
				))}
