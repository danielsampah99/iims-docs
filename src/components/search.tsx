...
const MODIFIER_KEY_REGEX = /(Mac|iPhone|iPod|iPad)/i;
...
function useAutocomplete({
	...
	const [autocomplete] = useState<Autocomplete>(() =>
		createAutocomplete<Result, SyntheticEvent, MouseEvent, KeyboardEvent>({
			id,
			...
			<Input
				...
				onKeyDown={(event) => {
					if (
						...
						// for those using mac books and the safari browser, closing the dialog with the escape key can sometimes cause the scroll position to jump to the
						// bottom of the page. This is a workaround for that until I can figure out a proper fix in Headless UI.
...
export function Search() {
	...
	useEffect(() => {
		setModifierKey(
			MODIFIER_KEY_REGEX.test(navigator.userAgent || navigator.platform)
				? "⌘"
				: "Ctrl ",
		);
	}, []);
